...
$listData = $db->sqa("SELECT * FROM {mytinytodo_lists} WHERE delta = ? AND entity_id = ?", array($delta, $entity_id));
...
$data = array();
$q = $db->dq("SELECT * FROM {mytinytodo_todolist} WHERE list_id = ? $sqlWhere ORDER BY ". $listData['_uid_field'] ." DESC LIMIT 100", array($listId));
...
function printRss($listData, $data)
{
	...
	$entity_id = (int)$_GET['eid'];
	$link = url("node/{$entity_id}", array('absolute' => true));
	$buildDate = gmdate('r');
