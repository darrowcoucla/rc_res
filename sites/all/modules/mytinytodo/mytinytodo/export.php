...
$listData = $db->sqa("SELECT * FROM {mytinytodo_lists} WHERE delta = ? AND entity_id = ? ". ($onlyPublishedList ? "AND published=1" : ""), array($delta, $entity_id) );
if (!$listData) {
...
$data = array();
$q = $db->dq("SELECT *, duedate IS NULL AS ddn FROM {mytinytodo_todolist} WHERE list_id = ? $sqlSort", array($listId));
