...
if(isset($_GET['loadLists']))
{
	...
	$entity_id = (int)$_GET['eid'];
	$t['total'] = 0;
	$q = $db->dq("SELECT * FROM {mytinytodo_lists} WHERE delta = ? AND entity_id = ? ORDER BY ow ASC, id ASC", array($delta, $entity_id));
...
elseif(isset($_GET['loadTasks']))
{
	...
	if($listId == -1) {
		...
		$sqlWhere .= " AND {mytinytodo_todolist}.list_id IN (". implode(array_keys($userLists), ','). ") ";
	}
	else $sqlWhere .= " AND {mytinytodo_todolist}.list_id=". $listId;
	...
	$t['total'] = 0;
	$t['list'] = array();
	$q = $db->dq("SELECT *, duedate IS NULL AS ddn FROM {mytinytodo_todolist} $inner WHERE 1=1 $sqlWhere $sqlSort");
	while($r = $q->fetch_assoc($q))
	{
		$t['total']++;
...
elseif(isset($_GET['newTask']))
{
	...
	$ow = 1 + (int)$db->sq("SELECT MAX(ow) FROM {mytinytodo_todolist} WHERE list_id=$listId AND compl=0");
	$db->ex("BEGIN");
	$id = db_insert('mytinytodo_todolist')
		->fields(array(
		    'uuid' => generateUUID(),
		    'list_id' => $listId,
		    'title' => $title,
	...
	if($tags != '')
	{
		...
		if($aTags) {
			...
			$db->ex("UPDATE {mytinytodo_todolist} SET tags=?,tags_ids=? WHERE id=$id", array(implode(',',$aTags['tags']), implode(',',$aTags['ids'])));
		}
	}
	$db->ex("COMMIT");
	$r = $db->sqa("SELECT * FROM {mytinytodo_todolist} WHERE id=$id");
	$t['list'][] = prepareTaskRow($r);
	$t['total'] = 1;
	jsonExit($t);
}
...
elseif(isset($_GET['completeTask']))
{
	...
	$listId = (int)$db->sq("SELECT list_id FROM {mytinytodo_todolist} WHERE id=$id");
	if($compl) 	$ow = 1 + (int)$db->sq("SELECT MAX(ow) FROM {mytinytodo_todolist} WHERE list_id=$listId AND compl=1");
	else $ow = 1 + (int)$db->sq("SELECT MAX(ow) FROM {mytinytodo_todolist} WHERE list_id=$listId AND compl=0");
	$dateCompleted = $compl ? time() : 0;
	$db->dq("UPDATE {mytinytodo_todolist} SET compl=$compl,ow=$ow,d_completed=?,d_edited=? WHERE id=$id",
...
elseif(isset($_GET['editNote']))
{
	...
	stop_gpc($_POST);
	$note = str_replace("\r\n", "\n", trim(_post('note')));
	$db->dq("UPDATE {mytinytodo_todolist} SET note=?,d_edited=? WHERE id=$id", array($note, time()) );
	$t = array();
	$t['total'] = 1;
...
elseif(isset($_GET['editTask']))
{
	...
	$listId = $db->sq("SELECT list_id FROM {mytinytodo_todolist} WHERE id=$id");
	$tags = trim(_post('tags'));
	$db->ex("BEGIN");
	...
	$db->dq("UPDATE {mytinytodo_todolist} SET title=?,note=?,prio=?,tags=?,tags_ids=?,duedate=?,d_edited=? WHERE id=$id",
...
elseif(isset($_GET['changeOrder']))
{
	...
	if($order)
	{
		...
		foreach($ad as $diff=>$ids) {
			...
			$db->dq("UPDATE {mytinytodo_todolist} SET $set,d_edited=? WHERE id IN (".implode(',',$ids).")", array(time()) );
		}
		$db->ex("COMMIT");
		$t['total'] = 1;
	}
...
elseif(isset($_GET['setPrio']))
{
	...
	if($prio < -1) $prio = -1;
	elseif($prio > 2) $prio = 2;
	$db->ex("UPDATE {mytinytodo_todolist} SET prio=$prio,d_edited=? WHERE id=$id", array(time()) );
	$t = array();
	$t['total'] = 1;
...
elseif(isset($_GET['renameList']))
{
	...
	$entity_id = trim(_post('eid'));
	$name = str_replace(array('"',"'",'<','>','&'),array('','','','',''),trim(_post('name')));
	$result = $db->dq("UPDATE {mytinytodo_lists} SET name=?,d_edited=? WHERE id=? AND delta=? AND entity_id=?", array($name, time(), $id, $delta, $entity_id) );
	$t['total'] = $result->affected();
	$r = $db->sqa("SELECT * FROM {mytinytodo_lists} WHERE id=$id AND delta = ? AND entity_id = ?", array($delta, $entity_id));
	$t['list'][] = prepareList($r);
	jsonExit($t);
}
elseif(isset($_GET['deleteList']))
{
	...
	if($t['total']) {
		...
		$db->ex("DELETE FROM {mytinytodo_todolist} WHERE list_id=$id");
	}
	$db->ex("COMMIT");
	jsonExit($t);
}
...
elseif(isset($_GET['moveTask']))
{
	...
	if($fromId == -1 && $result && $r = $db->sqa("SELECT * FROM {mytinytodo_todolist} WHERE id=$id")) {
		$t['list'][] = prepareTaskRow($r);
	}
	jsonExit($t);
}
...
elseif(isset($_GET['clearCompletedInList']))
{
	...
	$listId = (int)_post('list');
	$db->ex("BEGIN");
	$db->ex("DELETE FROM {mytinytodo_tag2task} WHERE task_id IN (SELECT id FROM {mytinytodo_todolist} WHERE list_id=? and compl=1)", array($listId));
	$result = $db->ex("DELETE FROM {mytinytodo_todolist} WHERE list_id=$listId and compl=1");
	$t['total'] = $result->affected();
	$db->ex("COMMIT");
	jsonExit($t);
}
...
function deleteTask($id)
{
	...
	$result = $db->dq("DELETE FROM {mytinytodo_todolist} WHERE id=$id");
...
function moveTask($id, $listId)
{
	...
	$r = $db->sqa("SELECT * FROM {mytinytodo_todolist} WHERE id=?", array($id));
	...
	$ow = 1 + (int)$db->sq("SELECT MAX(ow) FROM {mytinytodo_todolist} WHERE list_id=? AND compl=?", array($listId, $r['compl']?1:0));
	
	$db->ex("BEGIN");
	$db->ex("UPDATE {mytinytodo_tag2task} SET list_id=? WHERE task_id=?", array($listId, $id));
	$db->dq("UPDATE {mytinytodo_todolist} SET list_id=?, ow=?, d_edited=? WHERE id=?", array($listId, $ow, time(), $id));
	$db->ex("COMMIT");
	return true;
}
...
function getUserListsSimple()
{
	$db = DBConnection::instance();
	$a = array();
	$q = $db->dq("SELECT id,name FROM {mytinytodo_lists} ORDER BY id ASC");
	while($r = $q->fetch_row()) {
		$a[$r[0]] = $r[1];
	}
	return $a;
