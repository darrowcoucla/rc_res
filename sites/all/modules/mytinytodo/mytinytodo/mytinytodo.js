...
var mytinytodo = window.mytinytodo = _mtt = {
	...
	init: function(options)
	{
		...
		$("#edittags").autocomplete(base_path + 'mytinytodo/ajax?suggestTags', {scroll: false, multiple: true, selectFirst:false, max:8, extraParams:{list:function(){ var taskId = document.getElementById('taskedit_form').id.value; return taskList[taskId].listId; }}});
...
function exportCurList(format)
{
	...
	parent.location.href = base_path + 'mytinytodo/export?list=' + curList.id + '&format=' + format + '&delta=' + delta + '&eid=' + entity_id;
};

function feedCurList()
{
	if(!curList) return;
	parent.location.href = base_path + 'mytinytodo/feed?list=' + curList.id + '&delta=' + delta + '&eid=' + entity_id;
}
...
function showSettings()
{
	...
	$('#page_ajax').load(base_path + 'mytinytodo/settings?ajax=yes&delta=' + delta + '&eid=' + entity_id,null,function(){ 
...
{
	if(!frm) return false;
	var params = { save:'ajax', delta: delta, eid: entity_id };
