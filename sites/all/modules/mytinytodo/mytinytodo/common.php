...
function have_write_access($listId = null)
{
        ...
        $entity_id = (int)_post('eid');
        if (!$entity_id)
		$entity_id = (int)_get('eid');

        $node = (object)$db->dq("SELECT nid, uid, type FROM {node} WHERE nid = ?", $entity_id)->fetch_assoc();
...
class Config
{
	...
	public static function loadConfig()
        {
               if (!isset($_GET['eid']) || !isset($_GET['delta']))
                       return;

               $db = DBConnection::init(new Database_Drupal);
               self::$config = unserialize($db->sq("SELECT settings FROM {mytinytodo_lists} WHERE entity_id = ? AND delta = ?", array( (int)$_GET['eid'], (int)$_GET['delta'] )));
        }
		...
		db_update('mytinytodo_lists')
		  ->fields(array(
			'settings' => serialize(self::$config),
                  ...
                  ->condition('entity_id', self::$config['entity_id'])
		  ->condition('delta', self::$config['delta'])
		  ->execute();
	}    
}
